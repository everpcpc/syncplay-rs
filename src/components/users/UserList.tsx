  ...
  return (
    <div className="space-y-2">
      ...
      {users.length === 0 ? (
        ...
        <div className="space-y-2">
          {users.map((user) => (
            <div key={user.username} className="bg-gray-700 rounded p-2 text-sm">
              ...
              <div className="text-xs text-gray-400 mt-1">Room: {user.room}</div>

              {user.file && (
                <div className="text-xs text-gray-300 mt-1 truncate">File: {user.file}</div>
              )}

              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`text-xs px-2 py-0.5 rounded ${
                    user.isReady ? "bg-green-600 text-white" : "bg-gray-600 text-gray-300"
                  }`}
                >
